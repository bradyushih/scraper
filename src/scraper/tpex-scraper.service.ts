...
export class TpexScraperService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    // const tPexMarket = await this.fetchMarketTrades(date);
    // console.log(`tPexMarket:`);
    // console.log(tPexMarket);

    // const tPexMarketBreadth = await this.fetchMarketBreadth(date);
    // console.log(`tPexMarketBreadth:`);
    // console.log(tPexMarketBreadth);

    // const tPexMarketInvestorsTrades = await this.fetchInstInvestorsTrades(date);
    // console.log(`tPexMarketInvestorsTrades:`);
    // console.log(tPexMarketInvestorsTrades);

    // const tPexMarketMarginBalance = await this.fetchMarginTransactions(date);
    // console.log(`tPexMarketMarginBalance:`);
    // console.log(tPexMarketMarginBalance);
  ...
  async fetchMarketTrades(date: string) {
    ...
    const responseData = await firstValueFrom(this.httpService.get(url)).then(
      (response) => response.data.iTotalRecords > 0 && response.data,
    );
  ...
  async fetchMarketBreadth(date: string) {
    ...
    // 整理回應資料
    const { upNum, upStopNum, downNum, downStopNum, noChangeNum, noTradeNum } =
      responseData;
  ...
  async fetchMarginTransactions(date: string) {
    ...
    // 整理回應資料
    const raw = [...responseData.tfootData_one, ...responseData.tfootData_two] // 取出融資融券統計
  ...
  async fetchIndicesQuotes(date: string) {
    ...
    // 取得回應資料
    const responseData = await firstValueFrom(this.httpService.get(url)).then(
      (response) => (response.data.iTotalRecords > 0 ? response.data : null),
    );
  ...
  async fetchIndicesTrades(date: string) {
    ...
    // 計算電子工業成交量值
    const electronic = indices.reduce(
      (trades, data) => {
        return [
    ...
    const data = responseData.aaData
      ...
      .map((row) => {
        ...
        const referencePrice =
          (closePrice &&
            change !== null &&
            numeral(closePrice).subtract(change).value()) ||
          null;
