...
export class TaifexScraperService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    // const txfTrades = await this.fetchInstInvestorsTxfTrades(date);
    // console.log(`txfTrades`);
    // console.log(txfTrades);

    // const txoTrades = await this.fetchInstInvestorsTxoTrades(date);
    // console.log(`txoTrades`);
    // console.log(txoTrades);

    // const putCallRatio = await this.fetchTxoPutCallRatio(date);
    // console.log(`putCallRatio`);
    // console.log(putCallRatio);

    // const largeTraderTxfPosition =
    //   await this.fetchLargeTradersTxfPosition(date);
    // console.log(`largeTraderTxfPosition`);
    // console.log(largeTraderTxfPosition);

    // const largeTraderTxoPosition =
    //   await this.fetchLargeTradersTxoPosition(date);
    // console.log(`largeTraderTxoPosition`);
    // console.log(largeTraderTxoPosition);

    // const mxfMarketOi = await this.fetchMxfMarketOi(date);
    // console.log(`mxfMarketOi`);
    // console.log(mxfMarketOi);

    // const instInvestorsMxfOi = await this.fetchInstInvestorsMxfOi(date);
    // console.log(`instInvestorsMxfOi`);
    // console.log(instInvestorsMxfOi);

    // const retailMxfPosition = await this.fetchRetailMxfPosition(date);
    // console.log(`retailMxfPosition`);
    // console.log(retailMxfPosition);

    // const exchangeRates = await this.fetchExchangeRates(date);
  ...
  async fetchInstInvestorsTxfTrades(date: string) {
    ...
    const responseData = await firstValueFrom(
      ...
      csvtojson({ noheader: true, output: 'csv' }).fromString(
    ...
    const raw = [...dealers.slice(3), ...sitc.slice(3), ...fini.slice(3)].map(
      (data) => numeral(data).value(),
    );
  ...
  async fetchTxoPutCallRatio(date: string) {
    ...
    // 轉換為比率
    const txoPutCallVolumeRatio = numeral(txoPutCallVolumeRatioPercent)
      .divide(100)
      .value();
  ...
  async fetchLargeTradersTxfPosition(date: string) {
    ...
    const frontMonth = frontMonthRow
      .slice(5, -1)
      .map((data) => numeral(data).value());
    const frontMonthSpecific = frontMonthSpecificRow
      .slice(5, -1)
      .map((data) => numeral(data).value());
    const frontMonthNonSpecific = frontMonth.map(
      (data, i) => data - frontMonthSpecific[i],
    );
    const allMonths = allMonthsRow
      .slice(5, -1)
      .map((data) => numeral(data).value());
    const allMonthsSpecific = allMonthsSpecificRow
      .slice(5, -1)
      .map((data) => numeral(data).value());
    const allMonthsNonSpecific = allMonths.map(
      (data, i) => data - allMonthsSpecific[i],
    );
    const backMonths = allMonths.map((data, i) => data - frontMonth[i]);
    const backMonthsSpecific = allMonthsSpecific.map(
      (data, i) => data - frontMonthSpecific[i],
    );
    const backMonthsNonSpecific = backMonths.map(
      (data, i) => data - backMonthsSpecific[i],
    );
    ...
    const top5SpecificFrontMonthNetOi =
      top5SpecificFrontMonthLongOi - top5SpecificFrontMonthShortOi;
    const top10SpecificFrontMonthNetOi =
      top10SpecificFrontMonthLongOi - top10SpecificFrontMonthShortOi;
    const top5NonSpecificFrontMonthNetOi =
      top5NonSpecificFrontMonthLongOi - top5NonSpecificFrontMonthShortOi;
    const top10NonSpecificFrontMonthNetOi =
      top10NonSpecificFrontMonthLongOi - top10NonSpecificFrontMonthShortOi;
    ...
    const top5SpecificAllMonthsNetOi =
      top5SpecificAllMonthsLongOi - top5specificAllMonthsShortOi;
    const top10SpecificAllMonthsNetOi =
      top10SpecificAllMonthsLongOi - top10SpecificAllMonthsShortOi;
    const top5NonSpecificAllMonthsNetOi =
      top5NonSpecificAllMonthsLongOi - top5NonSpecificAllMonthsShortOi;
    const top10NonSpecificAllMonthsNetOi =
      top10NonSpecificAllMonthsLongOi - top10NonSpecificAllMonthsShortOi;
    ...
    const top5SpecificBackMonthsNetOi =
      top5SpecificBackMonthsLongOi - top5SpecificBackMonthsShortOi;
    const top10SpecificBackMonthsNetOi =
      top10SpecificBackMonthsLongOi - top10SpecificBackMonthsShortOi;
    const top5NonSpecificBackMonthsNetOi =
      top5NonSpecificBackMonthsLongOi - top5NonSpecificBackMonthsShortOi;
    const top10NonSpecificBackMonthsNetOi =
      top10NonSpecificBackMonthsLongOi - top10NonSpecificBackMonthsShortOi;
  ...
  async fetchLargeTradersTxoPosition(date: string) {
    ...
    // 將 string 型別數字轉換成 number 並計算出非特定人及遠月契約
    const txoCallFrontMonth = txoCallFrontMonthRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoCallFrontMonthSpecific = txoCallFrontMonthSpecificRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoCallFrontMonthNonSpecific = txoCallFrontMonth.map(
      (data, i) => data - txoCallFrontMonthSpecific[i],
    );
    const txoCallAllMonths = txoCallAllMonthsRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoCallAllMonthsSpecific = txoCallAllMonthsSpecificRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoCallAllMonthsNonSpecific = txoCallAllMonths.map(
      (data, i) => data - txoCallAllMonthsSpecific[i],
    );
    const txoCallBackMonths = txoCallAllMonths.map(
      (data, i) => data - txoCallFrontMonth[i],
    );
    const txoCallBackMonthsSpecific = txoCallAllMonthsSpecific.map(
      (data, i) => data - txoCallFrontMonthSpecific[i],
    );
    const txoCallBackMonthsNonSpecific = txoCallBackMonths.map(
      (data, i) => data - txoCallBackMonthsSpecific[i],
    );
    const txoCallFrontMonthMarketOi = numeral(
      txoCallFrontMonthRow.slice(-1),
    ).value();
    const txoCallAllMonthsMarketOi = numeral(
      txoCallAllMonthsRow.slice(-1),
    ).value();
    const txoCallBackMonthsMarketOi =
      txoCallAllMonthsMarketOi - txoCallFrontMonthMarketOi;
    const txoPutFrontMonth = txoPutFrontMonthRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoPutFrontMonthSpecific = txoPutFrontMonthSpecificRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoPutFrontMonthNonSpecific = txoPutFrontMonth.map(
      (data, i) => data - txoPutFrontMonthSpecific[i],
    );
    const txoPutAllMonths = txoPutAllMonthsRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoPutAllMonthsSpecific = txoPutAllMonthsSpecificRow
      .slice(6, -1)
      .map((data) => numeral(data).value());
    const txoPutAllMonthsNonSpecific = txoPutAllMonths.map(
      (data, i) => data - txoPutAllMonthsSpecific[i],
    );
    const txoPutBackMonths = txoPutAllMonths.map(
      (data, i) => data - txoPutFrontMonth[i],
    );
    const txoPutBackMonthsSpecific = txoPutAllMonthsSpecific.map(
      (data, i) => data - txoPutFrontMonthSpecific[i],
    );
    const txoPutBackMonthsNonSpecific = txoPutBackMonths.map(
      (data, i) => data - txoPutBackMonthsSpecific[i],
    );
    const txoPutFrontMonthMarketOi = numeral(
      txoPutFrontMonthRow.slice(-1),
    ).value();
    const txoPutAllMonthsMarketOi = numeral(
      txoPutAllMonthsRow.slice(-1),
    ).value();
    const txoPutBackMonthsMarketOi =
      txoPutAllMonthsMarketOi - txoPutFrontMonthMarketOi;
    ...
    // 計算臺指買權近月契約大額交易人淨部位
    const top5TxoCallFrontMonthNetOi =
      top5TxoCallFrontMonthLongOi - top5TxoCallFrontMonthShortOi;
    const top10TxoCallFrontMonthNetOi =
      top10TxoCallFrontMonthLongOi - top10TxoCallFrontMonthShortOi;
    const top5SpecificTxoCallFrontMonthNetOi =
      top5SpecificTxoCallFrontMonthLongOi -
      top5SpecificTxoCallFrontMonthShortOi;
    const top10SpecificTxoCallFrontMonthNetOi =
      top10SpecificTxoCallFrontMonthLongOi -
      top10SpecificTxoCallFrontMonthShortOi;
    const top5NonSpecificTxoCallFrontMonthNetOi =
      top5NonSpecificTxoCallFrontMonthLongOi -
      top5NonSpecificTxoCallFrontMonthShortOi;
    const top10NonSpecificTxoCallFrontMonthNetOi =
      top10NonSpecificTxoCallFrontMonthLongOi -
      top10NonSpecificTxoCallFrontMonthShortOi;

    // 計算臺指買權全部契約大額交易人淨部位
    const top5TxoCallAllMonthsNetOi =
      top5TxoCallAllMonthsLongOi - top5TxoCallAllMonthsShortOi;
    const top10TxoCallAllMonthsNetOi =
      top10TxoCallAllMonthsLongOi - top10TxoCallAllMonthsShortOi;
    const top5SpecificTxoCallAllMonthsNetOi =
      top5SpecificTxoCallAllMonthsLongOi - top5SpecificTxoCallAllMonthsShortOi;
    const top10SpecificTxoCallAllMonthsNetOi =
      top10SpecificTxoCallAllMonthsLongOi -
      top10SpecificTxoCallAllMonthsShortOi;
    const top5NonSpecificTxoCallAllMonthsNetOi =
      top5NonSpecificTxoCallAllMonthsLongOi -
      top5NonSpecificTxoCallAllMonthsShortOi;
    const top10NonSpecificTxoCallAllMonthsNetOi =
      top10NonSpecificTxoCallAllMonthsLongOi -
      top10NonSpecificTxoCallAllMonthsShortOi;

    // 計算臺指買權遠月契約大額交易人淨部位
    const top5TxoCallBackMonthsNetOi =
      top5TxoCallBackMonthsLongOi - top5TxoCallBackMonthsShortOi;
    const top10TxoCallBackMonthsNetOi =
      top10TxoCallBackMonthsLongOi - top10TxoCallBackMonthsShortOi;
    const top5SpecificTxoCallBackMonthsNetOi =
      top5SpecificTxoCallBackMonthsLongOi -
      top5SpecificTxoCallBackMonthsShortOi;
    const top10SpecificTxoCallBackMonthsNetOi =
      top10SpecificTxoCallBackMonthsLongOi -
      top10SpecificTxoCallBackMonthsShortOi;
    const top5NonSpecificTxoCallBackMonthsNetOi =
      top5NonSpecificTxoCallBackMonthsLongOi -
      top5NonSpecificTxoCallBackMonthsShortOi;
    const top10NonSpecificTxoCallBackMonthsNetOi =
      top10NonSpecificTxoCallBackMonthsLongOi -
      top10NonSpecificTxoCallBackMonthsShortOi;

    // 計算臺指賣權近月契約大額交易人淨部位
    const top5TxoPutFrontMonthNetOi =
      top5TxoPutFrontMonthLongOi - top5TxoPutFrontMonthShortOi;
    const top10TxoPutFrontMonthNetOi =
      top10TxoPutFrontMonthLongOi - top10TxoPutFrontMonthShortOi;
    const top5SpecificTxoPutFrontMonthNetOi =
      top5SpecificTxoPutFrontMonthLongOi - top5SpecificTxoPutFrontMonthShortOi;
    const top10SpecificTxoPutFrontMonthNetOi =
      top10SpecificTxoPutFrontMonthLongOi -
      top10SpecificTxoPutFrontMonthShortOi;
    const top5NonSpecificTxoPutFrontMonthNetOi =
      top5NonSpecificTxoPutFrontMonthLongOi -
      top5NonSpecificTxoPutFrontMonthShortOi;
    const top10NonSpecificTxoPutFrontMonthNetOi =
      top10NonSpecificTxoPutFrontMonthLongOi -
      top10NonSpecificTxoPutFrontMonthShortOi;

    // 計算臺指賣權全部契約大額交易人淨部位
    const top5TxoPutAllMonthsNetOi =
      top5TxoPutAllMonthsLongOi - top5TxoPutAllMonthsShortOi;
    const top10TxoPutAllMonthsNetOi =
      top10TxoPutAllMonthsLongOi - top10TxoPutAllMonthsShortOi;
    const top5SpecificTxoPutAllMonthsNetOi =
      top5SpecificTxoPutAllMonthsLongOi - top5SpecificTxoPutAllMonthsShortOi;
    const top10SpecificTxoPutAllMonthsNetOi =
      top10SpecificTxoPutAllMonthsLongOi - top10SpecificTxoPutAllMonthsShortOi;
    const top5NonSpecificTxoPutAllMonthsNetOi =
      top5NonSpecificTxoPutAllMonthsLongOi -
      top5NonSpecificTxoPutAllMonthsShortOi;
    const top10NonSpecificTxoPutAllMonthsNetOi =
      top10NonSpecificTxoPutAllMonthsLongOi -
      top10NonSpecificTxoPutAllMonthsShortOi;

    // 計算臺指賣權遠月契約大額交易人淨部位
    const top5TxoPutBackMonthsNetOi =
      top5TxoPutBackMonthsLongOi - top5TxoPutBackMonthsShortOi;
    const top10TxoPutBackMonthsNetOi =
      top10TxoPutBackMonthsLongOi - top10TxoPutBackMonthsShortOi;
    const top5SpecificTxoPutBackMonthsNetOi =
      top5SpecificTxoPutBackMonthsLongOi - top5SpecificTxoPutBackMonthsShortOi;
    const top10SpecificTxoPutBackMonthsNetOi =
      top10SpecificTxoPutBackMonthsLongOi -
      top10SpecificTxoPutBackMonthsShortOi;
    const top5NonSpecificTxoPutBackMonthsNetOi =
      top5NonSpecificTxoPutBackMonthsLongOi -
      top5NonSpecificTxoPutBackMonthsShortOi;
    const top10NonSpecificTxoPutBackMonthsNetOi =
  ...
  private async fetchInstInvestorsMxfOi(date: string) {
    ...
    // 計算三大法人小型臺指多方未平倉口數
    const instInvestorsMxfLongOi =
      dealersLongOiVolume + sitcLongOiVolume + finiLongOiVolume;

    // 計算三大法人小型臺指空方未平倉口數
    const instInvestorsMxfShortOi =
      dealersShortOiVolume + sitcShortOiVolume + finiShortOiVolume;
