...
export class TwseScraperService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    // const tradeMarket = await this.fetchMarketTrades(date);
    // console.log(`tradeMarket:`);
    // console.log(tradeMarket);

    // const marketBreadth = await this.fetchMarketBreadth(date);
    // console.log(`marketBreadth:`);
    // console.log(marketBreadth);

    // const investorTrade = await this.fetchInstInvestorsTrades(date);
    // console.log(`investorTrade`);
    // console.log(investorTrade);

    // const marginBalance = await this.fetchMarginTransactions(date);
    // console.log(`marginBalance`);
    // console.log(marginBalance);

    // const indicesQuotes = await this.fetchIndicesQuotes(date);
    // console.log(`indicesQuotes`);
    // console.log(indicesQuotes);

    // const indicesTrades = await this.fetchIndicesTrades(date);
    // console.log(`indicesTrades`);
    // console.log(indicesTrades);

    // const equitiesQuotes = await this.fetchEquitiesQuotes(date);
    // console.log(`equitiesQuotes`);
    // console.log(equitiesQuotes);

    // const equitiesInstInvestorsTrades =
    //   await this.fetchEquitiesInstInvestorsTrades(date);
    // console.log(`equitiesInstInvestorsTrades`);
    // console.log(equitiesInstInvestorsTrades);

    // const equitiesValues = await this.fetchEquitiesValues(date);
    // console.log(`equitiesValues`);
    // console.log(equitiesValues);
  }
  ...
  async fetchMarketTrades(date: string) {
    ...
    const responseData = await firstValueFrom(this.httpService.get(url)).then(
      (response) => response.data.stat === 'OK' && response.data,
    );
      ...
      responseData.data
        .map((row: [string, ...number[]]) => {
          ...
          const [tradeVolume, tradeValue, transaction, price, change] =
            values.map((value: number) => numeral(value).value());
          return {
            date: formattedDate,
            tradeVolume,
            tradeValue,
        ...
        .find((data: { date: string }) => data.date === date) || null; // 取得目標日期的成交資訊
  ...
  async fetchInstInvestorsTrades(date: string) {
    ...
    // 外資合計賣出金額
    const foreignInvestorsSell =
      foreignDealersExcludedSell + foreignDealersSell;

    // 計算外資合計買賣超
    const foreignInvestorsNetBuySell =
      foreignDealersExcludedNetBuySell + foreignDealersNetBuySell;
    ...
    // 計算自營商合計買賣超
    const dealersNetBuySell =
      dealersProprietaryNetBuySell + dealersHedgeNetBuySell;
  ...
  async fetchMarginTransactions(date: string) {
    ...
    // 計算融資餘額增減(仟元)
    const marginBalanceValueChange =
      marginBalanceValue - marginBalanceValuePrev;
  ...
  async fetchIndicesQuotes(date: string) {
    ...
    const data = _(quotes)
      ...
      .map((data: any[]) => {
        ...
        const changePercent = +numeral(change)
          .divide(referencePrice)
          .multiply(100)
          .format('0.00');
  ...
  async fetchEquitiesQuotes(date: string) {
    ...
    const data = responseData.data9.map((row) => {
      ...
      const referencePrice =
        closePrice && numeral(closePrice).subtract(change).value();

      // 計算漲跌幅
      const changePercent =
        closePrice &&
        +numeral(change).divide(referencePrice).multiply(100).format('0.00');
  ...
  async fetchEquitiesValues(date: string) {
    ...
    const data = responseData.data.reduce((tickers, row) => {
      ...
      const ticker = {
        ...
        dividendYear: numeral(dividendYear).value() + 1911, // 股利年度
      };
      return [...tickers, ticker];
    }, []);
