...
export class MopsScraperService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    const options = { market: 'sii' as Market, year: 2023, quarter: '1' };
    const quaterlyEps = await this.fetchQuarterlyEps(options);
    console.log(quaterlyEps);
  }
  ...
  async fetchQuarterlyEps(options: {
    ...
    // 取得 HTML 頁面
    const page = await firstValueFrom(this.httpService.post(url, form)).then(
